...
async def step(request: Request):
    body = await request.json()
    print(f"[App] Raw request body: {body}")
    action = DroneAction(**body)
    print(f"[App] Received step request (parsed): {action}")
...
async def get_memory_logs():
    episodes = []
    # Scan all task subdirectories (easy, medium, hard)
    for task_dir in ["easy", "medium", "hard"]:
        memory_path = BASE_DIR / "data" / task_dir / "memory.json"
    ...
    if not episodes:
        # Fallback to legacy path for backward compatibility during transition
        legacy_path = BASE_DIR / "data" / "memory.json"
    ...
    try:
        # Sort by reward or just show last few
        summary = []
        for ep in episodes[-10:]: # Show more since we aggregate
            summary.append({
...
async def predict(obs: DroneObservation):
    task_name = _env.state.task_name or "graders:grade_easy"
    action_dict = get_action_from_policy(obs, task_name)
